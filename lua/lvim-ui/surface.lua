    ...
    for _, pan in ipairs(state.panels) do
        ...
        if pan.provider and pan.provider.hide_cursor and not pan.provider.editable then
            local function list_move(delta)
                ...
                -- FOLD-AWARE step: move the hidden cursor by one DISPLAY line, so a closed fold counts
                -- as a single row (step OVER it in one press) instead of walking each hidden buffer
                -- line. `foldclosed`/`foldclosedend` are evaluated in the panel's own window. A panel
                -- WITHOUT folds returns -1 everywhere, so this reduces to the plain `line + delta`.
                api.nvim_win_call(pan.win, function()
                    local n = api.nvim_buf_line_count(pan.buf)
                    local line = api.nvim_win_get_cursor(pan.win)[1]
                    local target
                    if delta > 0 then
                        local fce = vim.fn.foldclosedend(line) -- last line of a closed fold on `line`, else -1
                        target = math.min((fce ~= -1 and fce or line) + 1, n)
                    else
                        local fc = vim.fn.foldclosed(line) -- first line of a closed fold on `line`, else -1
                        target = math.max((fc ~= -1 and fc or line) - 1, 1)
                    end
                    -- If the target lands INSIDE a closed fold, snap to its first (visible) line.
                    local dfc = vim.fn.foldclosed(target)
                    if dfc ~= -1 then
                        target = dfc
                    end
...
local function open_native_split(state)
    ...
    local function place_footer()
        ...
        -- Does another float overlap this panel's footer position? Two cases: a THIN bar (≤3 rows, e.g.
        -- the msgarea/statusline dock) → sit the footer ONE row above it (`chrome = 1`); a full PANEL
        -- dock (a bigger float that covers the panel bottom without spanning its whole height — e.g. the
        -- tasks panel over a full-height side tree) → the footer has no clean place, so HIDE it while the
        -- dock overlaps (a full-height BACKDROP spanning top→bottom is NOT a dock and is ignored).
        local chrome = 0
        local hide_covered = false
        for _, w in ipairs(api.nvim_list_wins()) do
            if w ~= state._footer_win and w ~= pan.win then
                ...
                if c.relative and c.relative ~= "" then
                    ...
                    if col_overlap and covers_bottom then
                        if fh <= 3 then
                            chrome = 1
                        elseif fr > text_top then -- a dock (not a full-height backdrop covering the top too)
                            hide_covered = true
                            break
                        end
                    end
        ...
        -- A dock covers the footer's row: hide the float (reopened when the dock closes) and reserve nothing.
        if hide_covered then
            if state._footer_win and api.nvim_win_is_valid(state._footer_win) then
                pcall(api.nvim_win_close, state._footer_win, false)
                state._footer_win = nil
            end
            pan.footer_reserve = 0
            vim.wo[pan.win].scrolloff = 0
            return
        end
    ...
    -- A dock (e.g. the tasks panel) opening or closing OVER this panel does not scroll or resize it,
    -- so watch window creation / destruction too — re-place (hide / re-show) the footer accordingly.
